...
_pg_circle_subtype_new3(PyTypeObject *type, double x, double y, double r) {
    pgCircleObject *circle_obj = (pgCircleObject *)pgCircle_Type.tp_new(type, NULL, NULL);
...
static PyObject *
pg_circle_str(pgCircleObject *self) { return pg_circle_repr(self); }
...
#define GETSET_FOR_SIMPLE(name) \
    static PyObject* \
    pg_circle_get##name(pgCircleObject *self, void *closure) { return PyFloat_FromDouble(self->circle.name); } \
    static int \
    pg_circle_set##name(pgCircleObject *self, PyObject *value, void *closure) { \
        double val; \
...
static PyObject *
pg_circle_getsafepickle(pgCircleObject *self, void *closure) { Py_RETURN_TRUE; }
...
static PyGetSetDef pg_circle_getsets[] = {
    ...
    {"__safe_for_unpickling__", (getter)pg_circle_getsafepickle, NULL, NULL, NULL},
...
pg_circle_init(pgCircleObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (!argcirc) {
        PyErr_SetString(PyExc_TypeError, "Argument must be Circle style object");
        return -1;
    }
    s_circ.x = argcirc->x;
    s_circ.y = argcirc->y;
